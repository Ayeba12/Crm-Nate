...
const LeadsList: React.FC<LeadsListProps> = ({
    ...
    return (
        <div className="bg-white border border-gray-200 rounded-lg flex flex-col h-[calc(100vh-140px)] animate-[fadeIn_0.3s_ease-out] dark:bg-gray-900 dark:border-gray-800 transition-colors">
            ...
            {/* Task Creation Modal via LeadsList */}
            <TaskModal
                isOpen={showTaskModal}
                onClose={() => { setShowTaskModal(false); setTaskLeadId(null); }}
                onSubmit={(taskData) => {
                    const newTask: Task = {
                        id: Math.random().toString(36).substr(2, 9),
                        ...
                        ownerId: 'Me',
                        leadIds: taskLeadId ? [taskLeadId] : (taskData.leadIds || []),
                        createdAt: new Date().toISOString()
                    };
                    onAddTask(newTask);
                ...
                leads={leads}
                initialData={taskLeadId ? { leadIds: [taskLeadId] } : {}}
            />
